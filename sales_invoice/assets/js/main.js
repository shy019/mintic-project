...
function calculate() {
  ...
  $('.invoicelist-body tbody tr').each(function () {
    ...
    var sum = ((rate * amount) + ((rate * amount) * (TAX_RATE / 100)));
    var tax = (rate * (TAX_RATE / 100));
  ...
  $('#total_price').text(new Intl.NumberFormat("CO-DC").format(total_price.toFixed(2)));
  $('#total_tax').text(new Intl.NumberFormat("CO-DC").format(total_tax.toFixed(2)));
